...
10.times do
	User.create(last_name:Faker::Name.last_name, first_name:Faker::Name.first_name, email: Faker::Internet.email, age:rand(12..100), description:Faker::Quote.famous_last_words, city_id:City.all.sample.id, password:Faker::Creature::Animal.name)
end
puts "Users created"

20.times do
	Gossip.create(title:Faker::Lorem.paragraph_by_chars(number: 10, supplemental: false), content:Faker::ChuckNorris.fact, user_id:User.all.sample.id)
end
...
20.times do
	PrivateMessage.create(content: Faker::Quotes::Shakespeare.romeo_and_juliet_quote, user_id: User.all.sample.id)
end
